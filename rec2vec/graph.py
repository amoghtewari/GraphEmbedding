...
def records_to_graph():
    ...
    for x in movies:
        ...
        for y in range(1,6):
            z=x+"_"+str(y)
            n = Node(id, z, "movie_rating")
            nodelist.append(n)
            nodedict[z]=n
            id=id+1

    def addNodes(nodeSource, type, idx):
        for z in nodeSource:
            n = Node(id, z, type)
            nodelist.append(n)
            nodedict[z]=n
    ...
    id1 = addNodes(directors, 'director', id)
    id2 = addNodes(actors, 'actor', id1)
    id3 = addNodes(genres, 'genre', id2)
    id4 = addNodes(ratings, 'user', id3)

    # Add edges between users and movie-rating nodes
    # YOUR CODE HERE

    def addNeighbors(node1, node2):
    ...
    for x in ratings:
        Y = ratings[x]
        for m in Y:
            addNeighbors(nodedict[x], nodedict[m + "_" + Y[m]])

    for x in movies:

        if (movies[x].director != None):
            addNeighbors(nodedict[movies[x].director], nodedict[x])

        for y in movies[x].actors:
            addNeighbors(nodedict[y], nodedict[x])

        for y in movies[x].genres:
            addNeighbors(nodedict[y], nodedict[x])

        for y in range(1,6):
            addNeighbors(nodedict[x+"_"+str(y)], nodedict[x])
